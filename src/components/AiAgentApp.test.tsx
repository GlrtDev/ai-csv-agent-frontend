...
import { initialAppState, type PromptResponse } from '../types/Types';
...
describe('AIAgentApp Component', () => {
  ...
  it('should render initial state correctly', () => {
    ...
    expect(screen.getByRole('heading', { name: /Send Prompt/i })).toBeInTheDocument();
  ...
  describe('File Upload', () => {
    ...
    it('should allow file selection and enable upload button', async () => {
      ...
      const chooseFileButton = screen.getByRole('button', { name: /Click to choose file/i });
      expect(chooseFileButton).toBeInTheDocument();

      const hiddenFileInput = screen.getByLabelText('Upload CSV File') as HTMLInputElement;
    ...
    it('should call handleFileUpload and show success on successful upload', async () => {
        mockHandleFileUpload.mockResolvedValueOnce({ access_token: 'fake_token', token_type: 'bearer' });
        renderComponent();
        ...
        await waitFor(() => expect(uploadButton).toHaveTextContent(/Processing.../i));

        await waitFor(() => expect(mockHandleFileUpload).toHaveBeenCalledWith(testFile, 'upload-csv'));
        await waitFor(() => expect(toast.success).toHaveBeenCalledWith('CSV uploaded successfully! You can now send a prompt.'));
        await waitFor(() => expect(screen.getByRole('button', { name: /File Uploaded/i })).toBeInTheDocument());
    ...
    it('should handle file upload failure', async () => {
      mockHandleFileUpload.mockRejectedValueOnce(new Error('Upload failed miserably'));
      renderComponent();
      ...
      await waitFor(() => expect(toast.error).toHaveBeenCalledWith('Error uploading CSV: Upload failed miserably'));
  ...
  describe('Send Prompt', () => {
    ...
    const simulateFileUpload = async () => {
        mockHandleFileUpload.mockResolvedValueOnce({ access_token: mockToken, token_type: 'bearer' });
            const hiddenFileInput = screen.getByLabelText('Upload CSV File') as HTMLInputElement;
            await userEvent.upload(hiddenFileInput, testFile);
    ...
    it('should call handleSendPromptRequest and display result on success', async () => {
      const mockApiResponse: PromptResponse = { data: { chartType: 'line', data: [{ x: 1, y: 2 }] }, text: 'Analysis complete' };
      ...
      expect(sendButton).toBeDisabled(); // Shows loading
          await waitFor(() => expect(sendButton).toHaveTextContent(/Processing.../i));

      await waitFor(() => expect(mockHandleSendPromptRequest).toHaveBeenCalledWith(promptText, mockToken));
      await waitFor(() => expect(toast.success).toHaveBeenCalledWith('Prompt processed successfully!'));
      await waitFor(() => expect(screen.getByText('Result')).toBeInTheDocument());
      await waitFor(() => expect(screen.getByText('Analysis complete')).toBeInTheDocument()); // resultText
      await waitFor(() => expect(screen.getByTestId('data-chart')).toBeInTheDocument()); // DataChart is rendered
    });

    it('should display raw JSON result if no chartType or data for chart', async () => {
        const mockApiResponse: PromptResponse = { data: { message: "This is a text result" }, text: 'Analysis complete (text only)' };
        mockHandleSendPromptRequest.mockResolvedValueOnce(mockApiResponse);
        ...
        await waitFor(async () => {
            ...
            expect(JSON.parse(resultPre.textContent || "")).toEqual(mockApiResponse.data);
        });
    });
    ...
    it('should show error if access token is missing', async () => {
      ...
      expect(toast.error).toHaveBeenCalledWith('Please upload a CSV file first to obtain an access token.');
      expect(mockHandleSendPromptRequest).not.toHaveBeenCalled();
    });

    it('should handle send prompt failure', async () => {
      mockHandleSendPromptRequest.mockRejectedValueOnce(new Error('Prompt failed spectacularly'));
      renderComponent();
      await simulateFileUpload();
      ...
      await waitFor(() => expect(toast.error).toHaveBeenCalledWith('Error sending prompt: Prompt failed spectacularly'));
      expect(screen.getByRole('button', { name: /Send Prompt/i })).toBeEnabled(); // Re-enabled
      expect(screen.getByText('Error')).toBeInTheDocument();
      expect(screen.getByText('Prompt failed spectacularly')).toBeInTheDocument();
    });
  });
});
