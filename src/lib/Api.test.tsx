...
describe('Api functions', () => {
  ...
  describe('handleFileUpload', () => {
    ...
    it('should throw an error if file upload fails with error detail', async () => {
      ...
      await expect(handleFileUpload(mockFile, 'upload-csv')).rejects.toThrow('Upload failed');
    });
    ...
    it('should throw a generic error if file upload fails without error detail', async () => {
      ...
      await expect(handleFileUpload(mockFile, 'upload-csv')).rejects.toThrow('Failed to upload CSV to upload-csv.');
    });
     it('should throw an error if response is not ok and json parsing fails', async () => {
      ...
      await expect(handleFileUpload(mockFile, 'upload-csv'))
        .rejects.toThrow('Failed to upload CSV to upload-csv.');
    });
  });
  ...
  describe('handleSendPromptRequest', () => {
    ...
    it('should send a prompt successfully and return data', async () => {
      const mockApiResponse: PromptResponse = { data: { result: 'success' }, text: 'Success text' };
      (fetch as vi.Mock).mockResolvedValueOnce({
        ok: true,
    ...
    it('should throw an error if token is missing', async () => {
      await expect(handleSendPromptRequest(mockPrompt, null)).rejects.toThrow('Authentication token is missing.');
    });
    ...
    it('should throw an error if sending prompt fails with detail', async () => {
      ...
      await expect(handleSendPromptRequest(mockPrompt, mockToken)).rejects.toThrow('API Error: Prompt processing failed');
    });
    ...
    it('should throw an error if sending prompt fails with string error', async () => {
      ...
      await expect(handleSendPromptRequest(mockPrompt, mockToken)).rejects.toThrow('API Error: A simple string error');
    });
    ...
    it('should throw a generic error if sending prompt fails without specific error detail', async () => {
      ...
      await expect(handleSendPromptRequest(mockPrompt, mockToken)).rejects.toThrow('Failed to send prompt. Status: 500');
    });
    ...
    it('should throw an error if server responds with non-JSON error', async () => {
      (fetch as vi.Mock).mockResolvedValueOnce({
        ok: false,
        json: async () => { throw new Error('Cannot parse JSON'); }, // Simulate error during response.json()
        status: 502,
        statusText: 'Bad Gateway',
      });

      await expect(handleSendPromptRequest(mockPrompt, mockToken))
    ...
    it('should throw an error if response data is missing expected fields', async () => {
      ...
      await expect(handleSendPromptRequest(mockPrompt, mockToken))
        .rejects.toThrow('Unexpected response format from the server. Missing "data" and/or "text" fields.');
    });
  });
});
